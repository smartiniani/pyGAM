...
def test_single_spline_penalty():
    ...
    """
    coef = np.array(1.0)
    assert np.alltrue(derivative(1, coef).toarray() == 0.0)
    assert np.alltrue(l2(1, coef).toarray() == 1.0)
    assert np.alltrue(monotonic_inc(1, coef).toarray() == 0.0)
    assert np.alltrue(monotonic_dec(1, coef).toarray() == 0.0)
    assert np.alltrue(convex(1, coef).toarray() == 0.0)
    assert np.alltrue(concave(1, coef).toarray() == 0.0)
    assert np.alltrue(none(1, coef).toarray() == 0.0)
